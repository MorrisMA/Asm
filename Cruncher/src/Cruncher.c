...
int main(int argc, char *argv[])
{
	...
	fwrite(&node_index, sizeof(long), 1, crunch_file);   // number of entries
...
void output_crunched_symtab(SYMTAB_NODE_PTR np)
{
	...
	node_index  = (long) np->info;

	fwrite(&node_index, sizeof(long), 1, crunch_file);
